...
console.info('process definition names', _.pluck(_store.processDefinition, 'name'));
...
function filter(src, data) {
  ...
  console.info('where filters', where);
  ...
  found = _.size(where.like) ? _.filter(found, function(item) {
    ...
    _.each(where.like, function(search, key) {
      ...
      console.info('testing....', realKey, item[realKey], keep);
    });
    return keep;
  }) : _.toArray(src);
...
HttpClient.prototype.get = function(path, options) {
  ...
  if (inPath('process-definition', path)) {
    found = filter(_store.processDefinition, options.data);
    console.info('filtered', options.data, found);

    if (path === 'process-definition/count') {
      results = {count: found.length};
    }
    else {
      results = _.toArray(found).slice(0, 10);
    }
  }

  // ...probably not correct...
  else if (inPath('process-instance', path)) {
    found = _.where(_store.processInstance, options.data);

    if (path === 'process-instance/count') {
      results = {count: found.length};
    }
    else {
