...
var HttpClient = function(config) {
  // console.info('Config cookie', config.cookie);
  // if (!config.cookie) {
  //   throw new Error('HttpClient needs a `cookie` configuration property.');
  // }
...
function reqAddCookie(self, req) {
  console.info('Config cookie', self.config.cookie);
  var cookie = self.config.cookie;
  if (cookie) {
    var val = typeof cookie === 'function' ? cookie() : cookie;
    console.info('Adding cookie', val);
    req.set('Cookie', val);
  }
  return req;
}

function end(self, done, stack) {
  return function(err, response) {
    if (!err && !response) {
      err = new Error('so screwed!');
    }
    ...
    if (err || (!response.ok && !response.noContent)) {
      ...
      console.info('error pre-request stack', stack);
      self.trigger('error', err, stack);
      return done(err);
    }
...
HttpClient.prototype.post = function(path, options) {
  ...
  reqAddCookie(self, req);
  req.end(end(self, done, (new Error().stack)));
};
