...
/** @lends AbstractClientResource */
{
  ...
  list: function(params, done) {
    ...
    var countPromise = this.count(params).then(
      function(count) {
        results.count = count;
      }
    );
    ...
    var listPromise = self.http.get(self.path, {data: params}).then(
      // Success
      function(itemsRes) {
    ...
    return Q.all([countPromise, listPromise]).then(
      function() {
        /**
  ...
  count: function(params, done) {
    ...
    return this.http.get(this.path +'/count', {data: params}).then(
      function(result){
