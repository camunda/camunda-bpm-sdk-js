...
_store.pile = {};
var localStoredPiles = localStorage.getItem('pile');
if (localStoredPiles) {
  log('piles from localStorage');
  _store.pile = JSON.parse(localStoredPiles);
}
else {
  _.each([
    {
  ...
  localStorage.setItem('pile', JSON.stringify(_store.pile));
}
