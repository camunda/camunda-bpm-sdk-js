...
CamundaForm.prototype.store = function(callback) {
  ...
  this.fireEvent('before-variables-store');
  ...
  try {
    ...
    // store them
    localStorage.setItem('camForm:'+ formId, JSON.stringify(this.variableManager.variables));
  }
  catch (error) {
...
CamundaForm.prototype.isRestorable = function() {
  ...
  if (!localStorage['camForm:'+ formId]) {
    return false;
  }
...
CamundaForm.prototype.restore = function(callback) {
  ...
  this.restorePrevented = false;
  if (!!this.restorePrevented) {
    // call callback??
    return;
  }
  ...
  for (var v in stored) {
    if (vars[v]) {
      for (var p in stored[v]) {
        // use the stored value when found
        vars[v][p] = stored[v][p] || vars[v][p];
      }
    }
    else {
      vars[v] = stored[v];
  ...
  // update the form fields with the restored values
  this.applyVariables();
  this.fireEvent('variables-restored');
  if(typeof callback === "function") {
    callback();
  }
...
CamundaForm.prototype.submit = function(callback) {
  ...
  localStorage.clearItem('camForm:'+ formId);
